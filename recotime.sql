...
RETURNS (USERID INTEGER, EMAIL VARCHAR(255), WORKTIME VARCHAR(255))
AS
BEGIN
  USERID = NULL;
  SELECT USERID, EMAIL, WORKTIME FROM USERS
  LEFT JOIN GROUPS ON USERS.GROUPID = GROUPS.GROUPID
  WHERE USERNAME = UPPER(:USERNAME) INTO :USERID, :EMAIL, :WORKTIME;
  IF (USERID IS NULL) THEN
  BEGIN
    WORKTIME = '9:30';
    USERID = GEN_ID(GEN_USERID, 1);
    INSERT INTO USERS (USERID, USERNAME, FULLNAME, CITYNAME, WORKTIME, GROUPID)
    VALUES (:USERID, UPPER(:USERNAME), :FULLNAME, :CITYNAME, :WORKTIME, 0);
