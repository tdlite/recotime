...
RETURNS (USERID INTEGER, EMAIL VARCHAR(255), STARTTIME VARCHAR(255))
AS
BEGIN
  USERID = NULL;
  SELECT USERID, EMAIL, STARTTIME FROM USERS
  LEFT JOIN GROUPS ON USERS.GROUPID = GROUPS.GROUPID
  LEFT JOIN UPTIME ON USERS.STARTID = UPTIME.STARTID
  WHERE USERNAME = UPPER(:USERNAME) INTO :USERID, :EMAIL, :STARTTIME;
  IF (USERID IS NULL) THEN
  BEGIN
    USERID = GEN_ID(GEN_USERID, 1);
    INSERT INTO USERS (USERID, USERNAME, FULLNAME, CITYNAME, GROUPID, STARTID)
