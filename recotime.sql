...
/* TABLE WRKMODES */
CREATE TABLE WRKMODES (
WRKMODEID INTEGER NOT NULL,
WRKMODETIME VARCHAR(255),
PRIMARY KEY (WRKMODEID));
...
RETURNS (USERID INTEGER, EMAIL VARCHAR(255), WRKMODETIME VARCHAR(255))
AS
BEGIN
  USERID = NULL;
  SELECT USERID, EMAIL, WRKMODETIME FROM USERS
  LEFT JOIN GROUPS ON USERS.GROUPID = GROUPS.GROUPID
  LEFT JOIN WRKMODES ON USERS.WRKMODEID = WRKMODES.WRKMODEID
  WHERE USERNAME = UPPER(:USERNAME) INTO :USERID, :EMAIL, :WRKMODETIME;
  IF (USERID IS NULL) THEN
  BEGIN
    USERID = GEN_ID(GEN_USERID, 1);
    INSERT INTO USERS (USERID, USERNAME, FULLNAME, CITYNAME, GROUPID, WRKMODEID)
    VALUES (:USERID, UPPER(:USERNAME), :FULLNAME, :CITYNAME, 0, 0);
    SELECT EMAIL FROM GROUPS
    WHERE GROUPID = 0 INTO :EMAIL;
    SELECT WRKMODETIME FROM WRKMODES
    WHERE WRKMODEID = 0 INTO :WRKMODETIME;
  END
END^
